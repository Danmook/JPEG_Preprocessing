...
class RCF(nn.Module):
    def __init__(self, rank, pretrained=None):
        ...
        self.weight_deconv2 = self._make_bilinear_weights( 4, 1).to(self.device)
        self.weight_deconv3 = self._make_bilinear_weights( 8, 1).to(self.device)
        self.weight_deconv4 = self._make_bilinear_weights(16, 1).to(self.device)
        self.weight_deconv5 = self._make_bilinear_weights(16, 1).to(self.device)
    ...
    def _make_bilinear_weights(self, size, num_channels):
        ...
        filt = (1 - abs(og[0] - center) / factor) * (1 - abs(og[1] - center) / factor)  # filt.shape:size*size
    ...
    def forward(self, x):
        ...
        pool1   = self.pool1(conv1_2)   # output 128*128
        conv2_1 = self.act(self.conv2_1(pool1))
        conv2_2 = self.act(self.conv2_2(conv2_1))
        pool2   = self.pool2(conv2_2)   # output 64*64
        conv3_1 = self.act(self.conv3_1(pool2))
        conv3_2 = self.act(self.conv3_2(conv3_1))
        conv3_3 = self.act(self.conv3_3(conv3_2))
        pool3   = self.pool3(conv3_3)   # output 32*32
        conv4_1 = self.act(self.conv4_1(pool3))
        conv4_2 = self.act(self.conv4_2(conv4_1))
        conv4_3 = self.act(self.conv4_3(conv4_2))
        pool4   = self.pool4(conv4_3)   # output 32*32
        conv5_1 = self.act(self.conv5_1(pool4))
        conv5_2 = self.act(self.conv5_2(conv5_1))
        conv5_3 = self.act(self.conv5_3(conv5_2))   # output: 512*32*32

        conv1_1_down = self.conv1_1_down(conv1_1)
        conv1_2_down = self.conv1_2_down(conv1_2)   # 21 * 128 *128
        conv2_1_down = self.conv2_1_down(conv2_1)
        conv2_2_down = self.conv2_2_down(conv2_2)   # 21*64*64
        conv3_1_down = self.conv3_1_down(conv3_1)
        conv3_2_down = self.conv3_2_down(conv3_2)   # 21*32*32
        conv3_3_down = self.conv3_3_down(conv3_3)
        conv4_1_down = self.conv4_1_down(conv4_1)
        conv4_2_down = self.conv4_2_down(conv4_2)
        conv4_3_down = self.conv4_3_down(conv4_3)   # 21*32*32
        conv5_1_down = self.conv5_1_down(conv5_1)
        conv5_2_down = self.conv5_2_down(conv5_2)
        conv5_3_down = self.conv5_3_down(conv5_3)   # 21*32*32

        out1 = self.score_dsn1(conv1_1_down + conv1_2_down) # 1* 128 *128
        out2 = self.score_dsn2(conv2_1_down + conv2_2_down) # 1*64*64
        out3 = self.score_dsn3(conv3_1_down + conv3_2_down + conv3_3_down)  # 1*32*32
        out4 = self.score_dsn4(conv4_1_down + conv4_2_down + conv4_3_down)  # 1*32*32
        out5 = self.score_dsn5(conv5_1_down + conv5_2_down + conv5_3_down)  # 1*32*32
