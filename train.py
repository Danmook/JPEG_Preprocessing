import os
import time
import argparse
import json
import torch
import torch.nn.functional as F
import random
from torch.utils.tensorboard import SummaryWriter
from torch.utils.data import DistributedSampler, DataLoader
import torch.multiprocessing as mp
from torch.distributed import init_process_group
from torch.nn.parallel import DistributedDataParallel
from env import AttrDict, build_env
from model_list import model_list, optimizer_list
from dataset import Dataset
from utils import scan_checkpoint, load_checkpoint, save_checkpoint

# Speed up when the model structure is fixed
torch.backends.cudnn.benchmark = True


def main_worker(rank, a, h):
    # Init DDP devices
    if h.num_gpus > 1:
        init_process_group(backend=h.dist_config['dist_backend'], init_method=h.dist_config['dist_url'],
                           world_size=h.dist_config['world_size'] * h.num_gpus, rank=rank)
    device = torch.device('cuda:{:d}'.format(rank))

    # Import model
    model = model_list(a, rank).to(device)

    # Print the model and the saving path
    save_path = os.path.join(a.checkpoint_path, a.model_name)
    if rank == 0:
        print(model)
        print("checkpoints directory : ", save_path)

    # Scan the checkpoints
    if os.path.isdir(save_path):
        com_cp = scan_checkpoint(save_path, a.model_name + '_')

    # Load the checkpoints
    if a.fine_tuning:
        if com_cp is None:
            raise Exception('No checkpoints found! Cannot finetune!')
        else:
            state_dict_com = load_checkpoint(com_cp, device)
            model.load_state_dict(state_dict_com['model'])
            steps = state_dict_com['steps'] + 1
            last_epoch = state_dict_com['epoch']
    else:
        state_dict_com = None
        steps = 0
        last_epoch = -1

    # Put the models to DDP
    if h.num_gpus > 1:
        model = DistributedDataParallel(model, device_ids=[rank], find_unused_parameters=True).to(device)

    # Init optimizer
    optim_com = optimizer_list(model, h)
    if a.fine_tuning and state_dict_com is not None:
        optim_com.load_state_dict(state_dict_com['optim_com'])
    scheduler_com = torch.optim.lr_scheduler.ExponentialLR(optim_com, gamma=h.lr_decay, last_epoch=last_epoch)

    # Load training set
    trainset = Dataset(a.training_dir, shuffle=True)
    train_sampler = DistributedSampler(trainset) if h.num_gpus > 1 else None
    train_loader = DataLoader(trainset, num_workers=h.num_workers, shuffle=False,
                              sampler=train_sampler,
                              batch_size=h.batch_size,
                              pin_memory=True,
                              drop_last=True)

    # Load validation set
    if rank == 0:
        validset = Dataset(a.validation_dir, shuffle=True)
        validation_loader = DataLoader(validset, num_workers=1, shuffle=False,
                                       sampler=None,
                                       batch_size=1,
                                       pin_memory=True,
                                       drop_last=True)

        # Init tensorboard
        sw = SummaryWriter(os.path.join(save_path, 'logs'))

    for epoch in range(max(0, last_epoch), a.epochs):
        if rank == 0:
            start = time.time()
            print("Epoch: {}".format(epoch + 1))

        # Use different random seed every epoch
        if h.num_gpus > 1:
            train_sampler.set_epoch(epoch)
        
        train(model, train_loader, optim_com, epoch, steps, a, h, rank, device, save_path, sw)
        if rank== 0:
            if steps % a.validation_interval == 0 and steps != 0:
                validation(model, steps, validation_loader, device, sw)
        scheduler_com.step()



def train(model, train_loader, optim_com, epoch, steps, a, h, rank, device, save_path, sw):
    # Start training
    model.train()
    # Main training programme
    for _, inputs in enumerate(train_loader):

        # Load one batch images
        inputs = torch.autograd.Variable(inputs.to(device, non_blocking=True))

        # Calculate loss
        """loss_items = compressor(img)
        loss, bit_rate, distortion = compressor.module.loss(img, loss_items) if h.num_gpus > 1 \
                                 else compressor.loss(img, loss_items)"""
        loss, _ = model(inputs)

        # Optimize
        optim_com.zero_grad()
        loss.backward()
        optim_com.step()
        if rank == 0:
            # STDOUT logging
            if steps % a.stdout_interval == 0:
                # print('Steps : {:d}, Bit rate : {:4.3f}, Distortion : {:4.3f}'.
                #         format(steps, bit_rate, distortion))
                print('Steps : {:d}, Distortion : {:4.3f}'.format(steps, loss))

            # Checkpointing
            if steps % a.checkpoint_interval == 0 and steps != 0:
                checkpoint_path = "{}/{}_{:08d}".format(save_path, a.model_name, steps)
                save_checkpoint(checkpoint_path,
                                {'model': (model.module if h.num_gpus > 1 else model).state_dict(),
                                 'optim_com': optim_com.state_dict(),
                                 'steps': steps,
                                 'epoch': epoch})

            # Tensorboard summary logging
            if steps % a.summary_interval == 0:
                sw.add_scalar("training/loss", loss, steps)
                # sw.add_scalar("training/bit_rate", bit_rate, steps)
                # sw.add_scalar("training/distortion", distortion, steps)

        steps += 1


def validation(model, steps, validation_loader, device, sw):
    # Validation
    model.eval()
    torch.cuda.empty_cache()
    val_err_distortion = 0

    with torch.no_grad():
        for j, inputs in enumerate(validation_loader):
            inputs = inputs.to(device, non_blocking=True)
            _, img_pre = model(inputs)
            img_pre = img_pre[0]
            val_err_distortion += F.mse_loss(inputs, img_pre).item()

        val_distortion = val_err_distortion / (j + 1)
        sw.add_scalar("validation/loss", val_distortion, steps)



def main():
    print('Initializing Training Process...')

    parser = argparse.ArgumentParser()

    '''
        '--model_name': Name of the model
        '--train_dir': Training data dir
        '--val_dir': Validation data dir
        '--checkpoint_path': Path to save your model
        '--config_file': Path of your config file
        '--epochs': Training epochs
        '--stdout_interval': The interval steps to log
        '--checkpoint_interval': The interval steps to save your model
        '--summary_interval': The interval steps to save your curves on tensorboard
        '--validation_interval': The interval steps to do validate
        '--fine_tuning': Finetune or not
        '--Lambda': The lambda setting for RD loss
    '''

    parser.add_argument('--model_name', default='preprocess', type=str)
    parser.add_argument('--train_dir', default=r'/data/liyao/vimeo/video_train/vimeo_train', type=str)
    parser.add_argument('--val_dir', default=r'/data/liyao/vimeo/vimeo_test/vimeo_test', type=str)
    parser.add_argument('--checkpoint_path', default='/output', type=str)
    parser.add_argument('--config_file', default=r'/configs/config.json', type=str)
    parser.add_argument('--epochs', default=1, type=int)
    parser.add_argument('--stdout_interval', default=5, type=int)
    parser.add_argument('--checkpoint_interval', default=5000, type=int)
    parser.add_argument('--summary_interval', default=100, type=int)
    parser.add_argument('--validation_interval', default=1000, type=int)
    parser.add_argument('--fine_tuning', default=False, type=bool)
    parser.add_argument('--Lambda', default=0.0483, type=float)

    a = parser.parse_args()

    # Create the path to save your model and copy the config file to the path
    with open(a.config_file) as f:
        data = f.read()
    json_config = json.loads(data)
    h = AttrDict(json_config)
    build_env(a.config_file, 'config.json', os.path.join(a.checkpoint_path, a.model_name))

    # Set the random seed and check the GPU nums
    torch.manual_seed(h.seed)
    random.seed(h.seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed(h.seed)
        h.num_gpus = torch.cuda.device_count()
        h.batch_size = int(h.batch_size / h.num_gpus)
        print('Batch size per GPU :', h.batch_size)
    else:
        pass

    # Main training function
    if h.num_gpus > 1:
        mp.spawn(main_worker, nprocs=h.num_gpus, args=(a, h))
    else:
        main_worker(0, a, h)


if __name__ == '__main__':
    main()

